...
public class NewAPI extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			...
			NewModel newModel = HttpUtil.of(request.getReader()).toModel(NewModel.class); // hàm HttpUtil.of trả về 1 HttpUtil, xong rồi gọi đến hàm toModel để trả về 1 model
			newModel.setCreatedBy(((UserModel) SessionUtil.getInstance().getValue(request,"USERMODEL")).getUserName());
			newModel = newService.save(newModel); // lưu cái model xuống db(thêm mới)
